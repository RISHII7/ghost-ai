...
async function requestProjectShare(projectId: string) {
  ...
  if (!response.ok) {
    throw new Error(
      await parseResponseError(response, "Unable to load access details"),
    );
  }
...
export function useProjectShare(projectId: string, open: boolean) {
  ...
  const invite = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await parseResponseError(response, "Unable to invite collaborator"),
        );
      }
  ...
  const remove = async (email: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          await parseResponseError(response, "Unable to remove collaborator"),
        );
      }
